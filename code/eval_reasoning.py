...
from utils.crop import crop as crop
import utils.keychain
import utils.globals as globals

openai.api_key = utils.keychain.OPENAI_API_KEY
choices = ["A", "B", "C", "D"]
...
def main(args):
    ...
    for engine in engines:
        ...
        engine_acc = {} # {subject: acc}

        for subject in subjects:
            dev_df = pd.read_csv(os.path.join(args.data_dir, "dev", subject + "_dev.csv"), header=None)[:args.ntrain]
            test_df = pd.read_csv(os.path.join(args.data_dir, "test", subject + "_test.csv"), header=None)

            # Specify test samples (the dev splits only have 5 examples per subject)
            # Retrieve the line from the index file
            with open(globals.MMLU_index_path, 'r') as f:
                index_file = json.load(f)

            for line in index_file:
                if line['task_name'] == subject:
                    test_indices = line['test']
...
if __name__ == "__main__":
    ...
    parser.add_argument("--data_dir", "-d", type=str, default="../data/reasoning")
    parser.add_argument("--save_dir", "-s", type=str, default="../results/outputs/reasoning")
